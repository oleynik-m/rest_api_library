...
type ItemRepo interface {
	...
	Update(item rest_api.Item) (int, error)
	Delete(id int) error
...
type ItemHistoryRepo interface {
	...
	GetValueByFilter(filter map[string]interface{}) (int,error)
}
...
func NewRepo (db *gorm.DB) *Repo {
	return &Repo{
		...
		DepartmentRepo: NewDepartmentPostgres(db),
		PublisherRepo:NewPublisherPostgres(db) ,
		ItemRepo: NewItemPostgres(db),
		UserRepo: NewUserPostgres(db),
		ItemHistoryRepo: NewItemHistoryPostgres(db),
	}
}